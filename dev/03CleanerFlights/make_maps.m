...
% % % % % %   drawnow
  ...
  %also store the full flight path, downsampled to ~20km
  Good= find(~isnan(Data.lat+Data.lon));
  a = [Data.lat,Data.lon];
  b = ones(size(Data.lat,1),2).*[Data.lat(min(Good)),Data.lon(min(Good))];
  x = nph_haversine(a,b);
  Results.Path.Lat{iFlight} = interp1(x(Good),Data.lat(Good),nanmin(x):20:nanmax(x)); 
  Results.Path.Lon{iFlight} = interp1(x(Good),Data.lon(Good),nanmin(x):20:nanmax(x));
...
save('flightpairs_identified_withpaths.mat','Results','Airports','Settings')
