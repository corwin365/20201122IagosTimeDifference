function [] = gh_regression2(Settings)
...
for iSeason=1:1:numel(Seasons)
  ...
  %use the first-listed index of the two as the climate index
  %since we have a tight maximum time separation between flights, and the
  %indices vary relatively slowly, this should be fine
  Flights = ThisSeason(:,1);
  ...
  for iLag=1:1:size(LagMatrix,1)
    ...
    if mdl.Rsquared.Adjusted > R2;
      ...
      %and store them
      Reg.Est(iSeason,:) = Coefs(2:end,1);
      Reg.SE( iSeason,:) = Coefs(2:end,2);
      Reg.T(  iSeason,:) = Coefs(2:end,3);
      Reg.P(  iSeason,:) = Coefs(2:end,4);
      
      %also store the number of flights used
      Reg.N(  iSeason,:) = mdl.NumObservations;
      
      %and the R2
      Reg.R2( iSeason,:) = mdl.Rsquared.Adjusted;
      R2 = mdl.Rsquared.Adjusted;
      
      %and the lag
      Reg.Lag(iSeason,:) = Lags;
...
for iSeason=1:1:numel(Seasons)
    ...
    %create panel
    k = k+1;
    subplot(numel(Seasons),1,k)
    
    
    %set axis lengths
    XRange = [-12,15]; TextLoc = -11.25;
    YRange = [0.5,size(Reg.Est,2)+1.5];
    ...
    for iIndex=1:1:numel(Settings.Indices)
      ...
      %get the value
      Value = Reg.Est(iSeason,iIndex).*MedianFlightTime.*2; %the 2 is due to how we normalised the indices back in routine "ff"
      
      %is it statistically significant?
      Sig   = Reg.P(iSeason,iIndex);
      if Sig < 0.05; Colour = Colours(iIndex,:); else Colour = 'w'; end      
      
      %indicate coefficient standard error
      StErr = Value + [-1,1] .* MedianFlightTime .* Reg.SE(iSeason,iIndex).*2;
...
%       setMarkerColor(h,Colours(iIndex,:),Alpha);      
  ...
  plot([0,0],[0,size(Reg.Est,2)+2],'-','linewi',2,'color',[1,1,1].*0.5)
  xlim(XRange)
  ylim(YRange)
  text(TextLoc,size(Reg.Est,2)+0.8,['(',Letters(k),') N=',num2str(Reg.N(iSeason,1)),'; R^2=',num2str(0.01.*round(100.*Reg.R2(iSeason,1)))],'fontsize',12)
